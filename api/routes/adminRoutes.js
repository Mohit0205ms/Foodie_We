...
// updating product
Router.post("/product/update/:id",Authorization,async(req,res)=>{
    console.log(req.params.id);
    try{
        const product = await Product.findById(req.params.id);
...
Router.post("/accept/table/booking",Authorization,async(req,res)=>{
    try{
...
Router.post("/reject/table/booking/:id",Authorization,async(req,res)=>{
    try{
        console.log(req.body);
